...
class FirestoreDatabase @Inject constructor(
    ...
    fun getUsers(userName: String): Flow<FirestoreResult<List<FirestoreUser>>> = callbackFlow {
        trySend(FirestoreResult.Loading)
        val listener = reference.whereEqualTo("name", userName).addSnapshotListener { value, error ->
    ...
    suspend fun createBackup(firestoreUser: FirestoreUser): Flow<FirestoreResult<String>> = callbackFlow {
    ...
    suspend fun updateBackup(firestoreUser: FirestoreUser): Flow<FirestoreResult<String>> = callbackFlow {
    ...
    suspend fun deleteBackup(firestoreUser: FirestoreUser): Flow<FirestoreResult<String>> = callbackFlow {
        if (firestoreUser.id != null) {
